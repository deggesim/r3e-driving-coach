...
export const buildPrompt = (
  ...
  if (significantZones.length > 0) {
    ...
    for (const zone of significantZones) {
      ...
      parts.push(header);
      parts.push(`- Velocità media: ${zone.avgSpeedKmh.toFixed(1)} km/h (min ${zone.minSpeedKmh.toFixed(1)})`);
      parts.push(`- Frenata max: ${(zone.maxBrakePct * 100).toFixed(0)}%`);
      parts.push(`- Acceleratore medio: ${(zone.avgThrottlePct * 100).toFixed(0)}%`);
  ...
  if (deviations && deviations.length > 0) {
    ...
    for (const dev of deviations) {
      ...
      const loc = cornerName ? `${cornerName} (@${dev.dist}m)` : `@${dev.dist}m`;
      parts.push(`- **${dev.type}** a ${loc}: ${dev.message} (delta: ${dev.delta.toFixed(1)})`);
    }
    parts.push('');
  } else if (deviations === null) {
    parts.push('## Nota');
    parts.push('Baseline non ancora calibrato — analisi standalone senza confronto con giri precedenti.');
    parts.push('');
  }
...
const buildBrakeTempSummary = (lap: LapRecord): string | null => {
  ...
  const fl = frames.map((f) => f.bt[0]).filter((t) => t !== BRAKE_TEMP.unavailable);
  const fr = frames.map((f) => f.bt[1]).filter((t) => t !== BRAKE_TEMP.unavailable);
  const rl = frames.map((f) => f.bt[2]).filter((t) => t !== BRAKE_TEMP.unavailable);
  const rr = frames.map((f) => f.bt[3]).filter((t) => t !== BRAKE_TEMP.unavailable);
  ...
  if (overheating.length > 0) {
    lines.push(`- ⚠ ${overheating.length} freni hanno superato la soglia critica di ${BRAKE_TEMP.max}°C`);
  }
